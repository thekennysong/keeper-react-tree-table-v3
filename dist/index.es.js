      ...
      var updatedRange = TreeState.sliceRows(source, from, to).map(function (model, i) {
        ...
        if (model.metadata.depth > 0 && !model.$state.isVisible) {
          ...
          if (!isStared && source.data.reduce(function (acc, row, index) {
            if (index >= source.data.map(function (row) {
              return row.data.name + row.data.type;
            }).indexOf(model.data.name + model.data.type) && acc !== 'false' && acc !== 'true') {
              if (row.data.isStared) {
                return acc + 'true';
              } else if (row.data.type == 'transactionEntity' || row.data.type == 'transaction') {
                return acc + 'false';
              } else {
                return acc + '';
              }
          ...
          }, '') !== 'true' || model.data.type === 'transaction' || model.data.type === 'transactionEntity') {
            model.$state.isVisible = false;
          }
        }
